...
void gs_buttom_check(void)
{
	static uint8_t cnt = 0;
	if (gs_s_msleep_is_timesup(&buttom_msleep))
	{
		if (Bit_RESET == GPIO_ReadInputDataBit(SOC0_LED_GPIO_PORT, SOC0_LED_GPIO_PIN))
		{
			if (cnt < 200)
			{
				cnt++;
		...
		gs_s_msleep_start_and_setting(&buttom_msleep, buttom_msleep.maxcnt);
	}
}
...
void gs_adc_dma_stop(void)
{
	//! DMA1_Channel1使能传输完成中断.
	DMA_ITConfig(B_VOLT_ADC_DMA_CHANNEL, DMA_IT_TC, DISABLE);

	//! 使能DMA1_Channel1.
	DMA_Cmd(B_VOLT_ADC_DMA_CHANNEL, DISABLE);

	//! Start ADC1 Software Conversion. 
	ADC_SoftwareStartConvCmd(B_VOLT_ADC_PORT, DISABLE);
}
...
void gs_calc_adc_data(void)
{
	...
	if (adcpara.flag_adc_sample == 0x01)
	{
		...
		#if 0
		configdata.para[GS_SENSOR_CHN_O2].a = 1.50;
		...
		//! O2.
		tmp = (float)(adcpara.filtered_data[2]) * 3.3f / 4095.0f;
		sensordata[GS_SENSOR_CHN_O2].val  = configdata.para[GS_SENSOR_CHN_O2].a * tmp + configdata.para[GS_SENSOR_CHN_O2].b;
		sensordata[GS_SENSOR_CHN_O2].val *= 100.0f;
		if (sensordata[GS_SENSOR_CHN_O2].val < 0.0f)
		{
			sensordata[GS_SENSOR_CHN_O2].val = 0.0f;
		}
		else if (sensordata[GS_SENSOR_CHN_O2].val > 30.0f)
		{
			sensordata[GS_SENSOR_CHN_O2].val = 30.0f;
		}
		
		//! CO.
		tmp = (float)(adcpara.filtered_data[3]) * 3.3f / 4095.0f;
		sensordata[GS_SENSOR_CHN_CO].val  = configdata.para[GS_SENSOR_CHN_CO].a * tmp + configdata.para[GS_SENSOR_CHN_CO].b;
		if (sensordata[GS_SENSOR_CHN_CO].val < 0.0f)
		{
			sensordata[GS_SENSOR_CHN_CO].val = 0.0f;
		}
		else if (sensordata[GS_SENSOR_CHN_CO].val > 1000.0f)
		{
			sensordata[GS_SENSOR_CHN_CO].val = 1000.0f;
		}
		
		//! H2S.
		tmp = (float)(adcpara.filtered_data[4]) * 3.3f / 4095.0f;
		sensordata[GS_SENSOR_CHN_H2S].val  = configdata.para[GS_SENSOR_CHN_H2S].a * tmp + configdata.para[GS_SENSOR_CHN_H2S].b;
		if (sensordata[GS_SENSOR_CHN_H2S].val < 0.0f)
		{
			sensordata[GS_SENSOR_CHN_H2S].val = 0.0f;
		}
		else if (sensordata[GS_SENSOR_CHN_H2S].val > 100.0f)
		{
			sensordata[GS_SENSOR_CHN_H2S].val = 100.0f;
		}
		
		//! LEL.
		tmp = (float)(adcpara.filtered_data[5]) * 3.3f / 4095.0f;
		sensordata[GS_SENSOR_CHN_LEL].val  = configdata.para[GS_SENSOR_CHN_LEL].a * tmp + configdata.para[GS_SENSOR_CHN_LEL].b;
		sensordata[GS_SENSOR_CHN_LEL].val *= 100.0f;
		if (sensordata[GS_SENSOR_CHN_LEL].val < 0.0f)
		{
			sensordata[GS_SENSOR_CHN_LEL].val = 0.0f;
		}
		else if (sensordata[GS_SENSOR_CHN_LEL].val > 100.0f)
		{
			sensordata[GS_SENSOR_CHN_LEL].val = 100.0f;
		}
	...
	#if 0
	if (gs_s_msleep_is_timesup(&adc_msleep))
    {
		if (0 == adcpara.flag_adc_sample)
		{
			gs_adc_dma_restart();
		}
		
        gs_s_msleep_stop(&adc_msleep);
        gs_s_msleep_start_and_setting(&adc_msleep, adc_msleep.maxcnt);
    }
	#endif
}
