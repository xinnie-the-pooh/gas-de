...
void ProcessComx(int fd)
{
	...
	while (1)
	{
		...
		else
		{
			...
			{
				tmp[11] = '\0';
				memset(phonenum1, '\0', sizeof(phonenum1));
				sprintf(phonenum1, "%s", tmp);
				SetLBOFF(45);
			}
		}
		
		memset(tmp, '\0', sizeof(tmp));
		CopyFromLW(230, tmp, 12);
		if (tmp[0] != '1')
		{
			SetLBON(47);
		}
		else
		{
			...
			{
				tmp[11] = '\0';
				memset(phonenum2, '\0', sizeof(phonenum2));
				sprintf(phonenum2, "%s", tmp);
				SetLBOFF(47);
			}
		}
		...
		if (1 == GetLWB(1, 0))
		{
			...
			if ((0 != warning_chn) && (warning_chn != last_warning_chn))
			{
				...
				else
				{
					...
					if (0 == gs_uart_send_message(fd, (unsigned char *)phonenum1, (unsigned char *)msg))
					{
					}
					else
					{
					}
					if (0 == gs_uart_send_message(fd, (unsigned char *)phonenum2, (unsigned char *)msg))
					{
					}
					else
					{
