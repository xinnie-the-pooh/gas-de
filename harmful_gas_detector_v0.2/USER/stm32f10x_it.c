...
/**
  ...
  * @retval None
  */
//! DEBUG TTL USART_IDLE
void UART4_IRQHandler(void)
{
	if (USART_GetITStatus(DEBUG_UART_PORT, USART_IT_IDLE) == SET)
	{
		//! 读取数据注意: 这句必须要, 否则不能够清除中断标志位.
		USART_ReceiveData(DEBUG_UART_PORT);
		//! 清除中断标志.
		USART_ClearITPendingBit(DEBUG_UART_PORT, USART_IT_IDLE);
		DMA_Cmd(DEBUG_UART_RX_DMA_CHANNEL, DISABLE);
		DMA_ClearFlag(DEBUG_UART_RX_DMA_FLAG);
		//! 算出接本帧数据长度.
		debuguart.rxlen = UART_RX_BUFF_SIZE - DMA_GetCurrDataCounter(DEBUG_UART_RX_DMA_CHANNEL);

		if (debuguart.rxlen > 0)
		{
			debuguart.flag_rx = 0x01;
		}

		DMA_SetCurrDataCounter(DEBUG_UART_RX_DMA_CHANNEL, UART_RX_BUFF_SIZE);
...
{
	//! 等待DMA1_Stream7传输完成.
	if (DMA_GetITStatus(DEBUG_UART_TX_DMA_IT_TC) != RESET)
	{
		//! 清除DMA1_Stream7传输完成标志.
		DMA_ClearITPendingBit(DEBUG_UART_TX_DMA_IT_TC);
		//! 关闭DMA.
		DMA_Cmd(DEBUG_UART_TX_DMA_CHANNEL, DISABLE);
	}
}

//! LORA TTL USART1_IDLE
