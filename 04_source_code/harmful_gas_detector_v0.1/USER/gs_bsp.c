...
void gs_work_led_init(LED_PARA *ledpara, uint16_t timesout_ms)
{
    ...
    //! 执行函数GPIO_PinRemapConfig前, 必须使能AFIO时钟.
...
void gs_alarm_led_init(LED_PARA *ledpara, uint16_t timesout_ms)
{
	...
	//! 使能GPIO的时钟.
	RCC_APB2PeriphClockCmd(ALARM_LED_RCC_PORT, ENABLE);

	GPIO_InitStruct.GPIO_Pin   = ALARM_LED_GPIO_PIN;	//! 管脚
	GPIO_InitStruct.GPIO_Mode  = GPIO_Mode_Out_PP;	//! 推挽输出模式.
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_2MHz;	//! 仅用于指示系统运行状态, 速度不用太高.
	GPIO_Init(ALARM_LED_GPIO_PORT, &GPIO_InitStruct);

	GPIO_SetBits(ALARM_LED_GPIO_PORT, ALARM_LED_GPIO_PIN);//! LED指示灯熄灭.
...
void gs_alarm_led_reverse(void)
{
	...
	if (status == 0x00)
	{
		GPIO_ResetBits(ALARM_LED_GPIO_PORT, ALARM_LED_GPIO_PIN);
		status = 0x01;
	}
	else if (status == 0x01)
	{
		GPIO_SetBits(ALARM_LED_GPIO_PORT, ALARM_LED_GPIO_PIN);
		status = 0x00;
	}
}

void gs_alarm_led_on(void)
{
	GPIO_ResetBits(ALARM_LED_GPIO_PORT, ALARM_LED_GPIO_PIN);
}

void gs_alarm_led_off(void)
{
	GPIO_SetBits(ALARM_LED_GPIO_PORT, ALARM_LED_GPIO_PIN);
}
...
void gs_alarm_beep_init(void)
{
	...
	GPIO_SetBits(ALARM_BEEP_GPIO_PORT, ALARM_BEEP_GPIO_PIN);//! LED指示灯熄灭.
}
...
void gs_jxm_switch_init(void)
{
	...
	//! 使能GPIO的时钟.
	RCC_APB2PeriphClockCmd(JXM_S0_RCC_PORT, ENABLE);
    RCC_APB2PeriphClockCmd(JXM_S1_RCC_PORT, ENABLE);

	GPIO_InitStruct.GPIO_Pin   = JXM_S0_GPIO_PIN;
	GPIO_InitStruct.GPIO_Mode  = GPIO_Mode_Out_PP;	//! 推挽输出模式.
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_2MHz;	//! 仅用于指示系统运行状态, 速度不用太高.
	GPIO_Init(JXM_S0_GPIO_PORT, &GPIO_InitStruct);
    
    GPIO_InitStruct.GPIO_Pin   = JXM_S1_GPIO_PIN;
	GPIO_InitStruct.GPIO_Mode  = GPIO_Mode_Out_PP;	//! 推挽输出模式.
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_2MHz;	//! 仅用于指示系统运行状态, 速度不用太高.
	GPIO_Init(JXM_S1_GPIO_PORT, &GPIO_InitStruct);

	GPIO_ResetBits(JXM_S0_GPIO_PORT, JXM_S0_GPIO_PIN);//! 默认不吸合.
    GPIO_ResetBits(JXM_S1_GPIO_PORT, JXM_S1_GPIO_PIN);//! 默认不吸合.
}

void gs_jxm_switch_ctrl(JXM_SWITCH jxm)
{
	if (JXM_SWITCH_O2 == jxm)
	{
		GPIO_ResetBits(JXM_S0_GPIO_PORT, JXM_S0_GPIO_PIN);
        GPIO_ResetBits(JXM_S1_GPIO_PORT, JXM_S1_GPIO_PIN);
	}
	else if (JXM_SWITCH_CO == jxm)
	{
		GPIO_SetBits(JXM_S0_GPIO_PORT, JXM_S0_GPIO_PIN);
        GPIO_ResetBits(JXM_S1_GPIO_PORT, JXM_S1_GPIO_PIN);
	}
    else if (JXM_SWITCH_H2S == jxm)
	{
		GPIO_ResetBits(JXM_S0_GPIO_PORT, JXM_S0_GPIO_PIN);
        GPIO_SetBits(JXM_S1_GPIO_PORT, JXM_S1_GPIO_PIN);
	}
    else if (JXM_SWITCH_LEL == jxm)
	{
		GPIO_SetBits(JXM_S0_GPIO_PORT, JXM_S0_GPIO_PIN);
        GPIO_SetBits(JXM_S1_GPIO_PORT, JXM_S1_GPIO_PIN);
	}
...
void gs_uart_init(UART_NUM uart_num, uint32_t baudrate, uint8_t parity)
{
	...
	if (uart_num == JXM_UART)
	{
		NVIC_InitStruct.NVIC_IRQChannel = JXM_UART_IRQ_CHN;
		...
		//! 使能GPIO的时钟.
		RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE);
		RCC_APB2PeriphClockCmd(JXM_UART_RCC_PORT, ENABLE);
		RCC_AHBPeriphClockCmd(JXM_UART_DMA_RCC_PORT, ENABLE);

		RCC_APB2PeriphClockCmd(JXM_UART_TX_RCC_PORT, ENABLE);
		GPIO_InitStruct.GPIO_Pin   = JXM_UART_TX_GPIO_PIN;
		GPIO_InitStruct.GPIO_Mode  = GPIO_Mode_AF_PP;
		GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(JXM_UART_TX_GPIO_PORT, &GPIO_InitStruct);

		RCC_APB2PeriphClockCmd(JXM_UART_RX_RCC_PORT, ENABLE);
		GPIO_InitStruct.GPIO_Pin   = JXM_UART_RX_GPIO_PIN;
		GPIO_InitStruct.GPIO_Mode  = GPIO_Mode_IN_FLOATING;
		GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_Init(JXM_UART_RX_GPIO_PORT, &GPIO_InitStruct);

		USART_DeInit(JXM_UART_PORT);
		...
		USART_Init(JXM_UART_PORT, &USART_InitStruct);

		USART_ITConfig(JXM_UART_PORT, USART_IT_IDLE, ENABLE);
//		USART_DMACmd(JXM_UART_PORT, USART_DMAReq_Tx, ENABLE);
		USART_DMACmd(JXM_UART_PORT, USART_DMAReq_Rx, ENABLE);

		USART_Cmd(JXM_UART_PORT, ENABLE);

        #if 0
		//! USART_TX_DMA
		NVIC_InitStruct.NVIC_IRQChannel = JXM_UART_TX_DMA_IRQ_CHN;
		NVIC_InitStruct.NVIC_IRQChannelPreemptionPriority = 0x00;
		...
		#if 0
		//! USART_RX_DMA
		NVIC_InitStruct.NVIC_IRQChannel = JXM_UART_RX_DMA_IRQ_CHN;
		NVIC_InitStruct.NVIC_IRQChannelPreemptionPriority = 0x00;
		NVIC_InitStruct.NVIC_IRQChannelSubPriority = 0x00;
		NVIC_InitStruct.NVIC_IRQChannelCmd = ENABLE;
		NVIC_Init(&NVIC_InitStruct);
		...
		#if 0
		//! USART_TX_DMA
		DMA_DeInit(JXM_UART_TX_DMA_CHANNEL);
		DMA_InitStruct.DMA_PeripheralBaseAddr = (uint32_t)&(JXM_UART_PORT->DR);
		...
		DMA_Init(JXM_UART_TX_DMA_CHANNEL, &DMA_InitStruct);
		DMA_ITConfig(JXM_UART_TX_DMA_CHANNEL, DMA_IT_TC, ENABLE);
		#endif

		//! USART_RX_DMA
		DMA_DeInit(JXM_UART_RX_DMA_CHANNEL);
		DMA_InitStruct.DMA_PeripheralBaseAddr = (uint32_t)&(JXM_UART_PORT->DR);
		DMA_InitStruct.DMA_MemoryBaseAddr     = (uint32_t)&jxmuart.rxdata[0];
		...
		DMA_InitStruct.DMA_Priority           = DMA_Priority_VeryHigh;
		DMA_InitStruct.DMA_M2M                = DMA_M2M_Disable;
		DMA_Init(JXM_UART_RX_DMA_CHANNEL, &DMA_InitStruct);
	...
	else if (uart_num == LORA_UART)
	{
		...
		//! 使能GPIO的时钟.
		RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE);
		RCC_APB1PeriphClockCmd(LORA_UART_RCC_PORT, ENABLE);
...
void gs_uart_send(UART_NUM uart_num, uint8_t *buff, uint16_t len)
{
	...
	if (uart_num == JXM_UART)
	{
		memcpy(jxmuart.txdata, buff, len);
		...
		for (i = 0; i < len; i++)
		{
			while (USART_GetFlagStatus(JXM_UART_PORT, USART_FLAG_TC) != SET);
			USART_SendData(JXM_UART_PORT, (unsigned char)buff[i]);
		}
		while (USART_GetFlagStatus(JXM_UART_PORT, USART_FLAG_TC) != SET);
		#endif
	}
...
int gs_uart_recv(UART_NUM uart_num, uint8_t *buff, uint16_t *len)
{
	if (uart_num == JXM_UART)
	{
		if (jxmuart.flag_rx == 1)
		{
			memcpy(buff, jxmuart.rxdata, jxmuart.rxlen);
			*len = jxmuart.rxlen;
			jxmuart.flag_rx = 0;
			return (0);
		}
...
void gs_adc_init(void)
{
	...
	ADC_RegularChannelConfig(B_VOLT_ADC_PORT, ADC_Channel_14, 1, ADC_SampleTime_239Cycles5);

	//! ADC1 regular channel16 configuration.
	ADC_RegularChannelConfig(B_VOLT_ADC_PORT, ADC_Channel_16, 2, ADC_SampleTime_239Cycles5);
...
void gs_calc_adc_data(void)
{
	...
	if (adcpara.flag_adc_sample == 0x01)
	{
		...
		syspara.battery_volt = (float)adcpara.filtered_data[0] * 3.3f / 4095.0f * 12.2f / 2.2f;
		syspara.mcu_temp     = (float)adcpara.filtered_data[1] * 3.3f / 4095.0f;
		syspara.mcu_temp     = (1.43f - syspara.mcu_temp) / 0.0043f + 25.0f;
//		gs_printf("syspara.battery_volt=%f V, syspara.mcu_temp=%f ℃.\r\n", syspara.battery_volt, syspara.mcu_temp);
		gs_adc_dma_restart();
